...
def run_pipeline(input_image_filename):
    ...
    if not os.path.exists(input_image_path):
        print(
            f'[-] Input file does not exist. Check file extension and whether it exists in the "input" directory'
        )
        return

    image_name = input_image_filename.rsplit(".", 1)[0]

    color_json_path = os.path.join(
        config.DIRECTORIES["JSON"], f"{image_name}_{int(time())}.json"
    )
